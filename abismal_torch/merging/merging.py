...
from abismal_torch.prior import PriorBase
...
class VariationalMergingModel(torch.nn.Module):
    def __init__(
        ...
        standardization_count_max: Optional[int] = 2_000,
        **kwargs,
    ) -> None:
        """
    ...
    def forward(self, inputs: Tuple[torch.Tensor, ...]) -> dict:
        ...
        for op in self.reindexing_ops:
            _hkl = op(hkl_in)
            _ipred = self.surrogate_posterior.rac.gather(z.T, asu_id, _hkl)
            _ipred = torch.square(_ipred) * scale

            _ll = self.likelihood(_ipred, iobs, sigiobs)
            _ll = self.average_by_images(_ll, image_id)
            ...
            if ll is None:
                ...
                idx = _ll > ll
                ipred = torch.where(idx, _ipred, ipred)
                ll = torch.where(idx, _ll, ll)
                hkl = torch.where(idx, _hkl, hkl)

        ipred_avg = torch.mean(ipred, dim=-1)
        return {
            "ipred_avg": ipred_avg,
            "loss_nll": -ll.mean(),
            "loss_kl": kl_div.mean(),
