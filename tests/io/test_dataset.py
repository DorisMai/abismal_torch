...
@pytest.mark.parametrize(
    ...
    [
        "stills",
        pytest.param(
            "mtz", marks=pytest.mark.xfail(reason="Mtz does not have lazy length")
        ),
    ],
)
...
def test_stills_dataset_cell_assignment(file_type, num_dials_stills, get_dataset):
    def f(ds):
        ds.cell = [34.0, 45.0, 98.0, 90.0, 90.0, 90.0]
...
@pytest.mark.parametrize("dmin", [1.0, 3.0, 5.0])
@pytest.mark.parametrize("file_type", ["stills"])
